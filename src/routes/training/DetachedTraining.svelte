<script lang="ts">
  /**
   * DetachedTraining Component
   *
   * Full training mode UI - handles game loop, input, rendering, and playback controls.
  ...
  interface Props {
    ...
    fspkBytes: Uint8Array | null;
    renderAssets: CharacterAssets | null;
    renderAssetsLoading: boolean;
    renderAssetsError: string | null;
  }
  ...
  function tickOneFrame() {
    ...
    try {
      ...
      sessionError = `WASM error: ${e instanceof Error ? e.message : String(e)}`;
      return;
    }
