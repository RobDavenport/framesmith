...
pub struct UpdateStateParam {
    ...
    pub state_data: crate::schema::State,
}
...
impl FramesmithMcp {
    ...
    async fn list_characters(&self) -> Result<CallToolResult, McpError> {
        use crate::commands::list_characters as list_chars;
    ...
    async fn list_global_states(&self) -> Result<CallToolResult, McpError> {
        ...
        for name in &states {
            let state = crate::globals::load_global_state(&project_dir, name).ok();
